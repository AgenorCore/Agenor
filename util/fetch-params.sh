...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/AgenorParams"
    else
        PARAMS_DIR="$HOME/.agenor-params"
    fi
fi
...
Failed to fetch the Agenor zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
Agenor - fetch-params.sh

This script will fetch the Agenor zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Agenor zkSNARK parameters. Note that it is
