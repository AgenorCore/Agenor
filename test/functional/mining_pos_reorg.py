...
class ReorgStakeTest(AgenorCoinTestFramework):
    ...
    def check_money_supply(self, expected_age, expected_zage):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected AGE and zAGE supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_age))
            for denom in node['zAGEsupply']:
                assert_equal(node['zAGEsupply'][denom], DecimalAmt(expected_zage[denom]))
    ...
    def run_test(self):
        ...
        # Check AGE and zAGE supply at the beginning
        # ------------------------------------------
        # zAGE supply: 2 coins for each denomination
        expected_zage_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # AGE supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zage_supply)
        ...
        # Verify that AGE and zAGE supplies were properly updated after the spends and reorgs
        self.log.info("Check AGE and zAGE supply...")
        ...
        expected_zage_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zage_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zage_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zage_supply)
