...
class ZerocoinSpendTest(AgenorCoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zage_bal, age_bal):
            zage_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zage_bal)
            age_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], age_bal)
            return zage_bal, age_bal
        ...
        zage_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zage_balance, 6666)
        ...
        zage_balance, balance = check_balances(denom_2, zage_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zage_balance, balance = check_balances(denom_3, zage_balance, balance)
        ...
        _, _ = check_balances(denom_1, zage_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
