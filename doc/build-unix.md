UNIX BUILD NOTES
====================
Some notes on how to build Agenor Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile Agenor Core and the dependencies,
...
This will build agenor-qt as well, if the dependencies are met.
...
memory available when compiling Agenor Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~agenor/+archive/agenor).
...
To build Agenor Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build agenor-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a agenor-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip agenord" to strip the debug
...
Security
--------
To help make your Agenor Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, Agenor Core should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, Agenor Core may be compiled in
