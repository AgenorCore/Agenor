...
#include "qt/agenor/forms/ui_send.h"
#include "qt/agenor/addnewcontactdialog.h"
#include "qt/agenor/qtutils.h"
#include "qt/agenor/sendchangeaddressdialog.h"
...
SendWidget::SendWidget(AgenorGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-age");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination ageAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(ageAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(ageAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
