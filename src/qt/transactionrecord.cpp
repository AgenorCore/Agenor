        ...
        QList<TransactionRecord>& parts)
{
    ...
    if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zageTracker->HasMintTx(hash))) {
        //zPIV stake reward
        sub.involvesWatchAddress = false;
        sub.type = TransactionRecord::StakeZAGE;
                                           ...
                                           QList<TransactionRecord>& parts)
{
    ...
    for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
        ...
        if (txout.IsZerocoinMint()) {
            ...
            sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
            sub.type = TransactionRecord::ZerocoinSpend_Change_zAge;
            sub.address = getValueOrReturnEmpty(wtx.mapValue, "zerocoinmint");
            if (!fFeeAssigned) {
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZAGEModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
...
bool IsZAGEType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZAGE:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zAge:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZAGE ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZAGE);
}
