...
const char * const Agenor_CONF_FILENAME = "agenor.conf";
const char * const Agenor_PID_FILENAME = "agenor.pid";
const char * const Agenor_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Agenor";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AgenorParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "AgenorParams";
#else
    // Unix
    return pathRet / ".agenor-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", Agenor_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", Agenor_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty agenor.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override agenor.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", Agenor_PID_FILENAME));
