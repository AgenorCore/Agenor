...
enum WalletFeature {
    ...
    FEATURE_PRE_Agenor = 61000, // inherited version..
...
enum ZerocoinSpendStatus {
    ZAGE_SPEND_OKAY = 0,                            // No error
    ZAGE_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZAGE_WALLET_LOCKED = 2,                         // Wallet was locked
    ZAGE_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZAGE_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZAGE_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZAGE_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZAGE_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZAGE_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZAGE_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZAGE_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZAGE_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZAGE_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZAGE_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZAGE_SPENT_USED_ZAGE = 14,                      // Coin has already been spend
    ZAGE_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZAGE_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void doZAgeRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZAGE);
    ...
    bool CreateZAGEOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zPIV wallet
    std::unique_ptr<CzAGETracker> zageTracker{nullptr};
    void setZWallet(CzAGEWallet* zwallet);
    CzAGEWallet* getZWallet();
