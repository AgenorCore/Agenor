...
bool CLegacyZAgeStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZAgeStake from non zc-spend", __func__);
    ...
    *this = CLegacyZAgeStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zage stake origin", __func__);

    // All good
    return true;
}

CLegacyZAgeStake::CLegacyZAgeStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZAgeStake::GetIndexFrom()
{
...
CAmount CLegacyZAgeStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZAgeStake::GetUniqueness() const
{
...
bool CLegacyZAgeStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zAGE stake block: height %d outside range", __func__, nHeight);
