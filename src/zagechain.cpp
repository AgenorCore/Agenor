...
{
    CDataStream serializedCoinSpend = ZAGEModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZAgenorupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
...
bool UpdateZAgenorupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
