...
#if defined(NDEBUG)
#error "Agenor cannot be compiled without assertions."
#endif
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zAGE mint output in tx %s",
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) return 5000000 * COIN;
    
    // Testnet high-inflation blocks [2, 200] with value 250k AGE
    ...
    // Subsidy is cut in half every 250,000 blocks which will occur approximately every 1 year.
    nSubsidy >>= halvings;
    return nSubsidy;    
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZAGEModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (!UpdateZAgenorupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zAGE supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("agenor-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    PublicCoinSpend publicSpend(params);
                    if (!ZAGEModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
    ...
    if (!UpdateZAgenorupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zAGE supply for block=%s height=%d", __func__,
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateAgenorupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zAGE serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // AgenorCoin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nAgenorCoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nAgenorCoinBadBlockBits)) {
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> ageInputs;
        std::vector<CTxIn> zAGEInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zAGEInputs.push_back(stakeIn);
            }else{
                ageInputs.push_back(stakeIn);
            }
        }
        const bool hasAGEInputs = !ageInputs.empty();
        const bool hasZAGEInputs = !zAGEInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasAGEInputs) {
                    ...
                    for (const CTxIn& ageIn : ageInputs)
                        if(ageIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            if(hasZAGEInputs) {
                for (const CTxIn& zAgeInput : zAGEInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zAgeInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zage");
